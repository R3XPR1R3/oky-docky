    ...
    <div className="min-h-screen">
      ...
      <div className="container mx-auto px-4 py-16 max-w-6xl">
        <motion.div initial={{ y: 20, opacity: 0 }} animate={{ y: 0, opacity: 1 }} transition={{ duration: 0.5 }} className="text-center mb-12">
          <h1 className="text-5xl font-bold mb-4">{t('selection.title')}</h1>
          <p className="text-xl text-slate-600 max-w-2xl mx-auto">{t('selection.description')}</p>
        </motion.div>

        <motion.div initial={{ y: 20, opacity: 0 }} animate={{ y: 0, opacity: 1 }} transition={{ duration: 0.5, delay: 0.05 }} className="max-w-xl mx-auto mb-8">
          <div className="relative">
            <Search className="absolute left-4 top-1/2 -translate-y-1/2 w-5 h-5 text-slate-400" />
            <Input value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} placeholder={t('selection.searchPlaceholder')} className="pl-12 py-6 text-lg rounded-xl border-2 focus:border-indigo-500 transition-colors" />
          </div>
        </motion.div>
        ...
        {!loading && templates.length === 0 && (
          <div className="text-center py-24">
            <p className="text-xl text-slate-500">{t('selection.noFormsFound')}</p>
            <p className="text-slate-400 mt-2">{t('selection.tryDifferent')}</p>
          </div>
        )}
              ...
              <motion.div key={template.id} initial={{ y: 20, opacity: 0 }} animate={{ y: 0, opacity: 1 }} transition={{ duration: 0.5, delay: index * 0.1 }} onHoverStart={() => setHoveredId(template.id)} onHoverEnd={() => setHoveredId(null)} onClick={() => onSelectDocument(template)} className="relative group cursor-pointer">
                <div className="relative bg-white rounded-2xl p-8 shadow-lg border-2 border-slate-200 hover:border-indigo-400 transition-all duration-300 hover:shadow-xl">
                  ...
                  <div className="flex items-start justify-between mb-6">
                    ...
                    <motion.div animate={{ x: hoveredId === template.id ? 5 : 0 }} transition={{ duration: 0.2 }}>
                      <ChevronRight className="w-6 h-6 text-slate-400 group-hover:text-indigo-600 transition-colors" />
                    </motion.div>
                  </div>
                  <h3 className="text-2xl font-bold mb-2 group-hover:text-indigo-600 transition-colors">{template.title}</h3>
                  ...
                  {template.tags && template.tags.length > 0 && (
                    <div className="flex flex-wrap gap-2 mt-4">
                      {template.tags.slice(0, 4).map((tag) => (
                        <span key={tag} className="text-xs px-2 py-1 rounded-full bg-slate-100 text-slate-500">{tag}</span>
                      ))}
                    </div>
                  )}
